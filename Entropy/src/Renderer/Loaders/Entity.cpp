...
void LoadZone::load_entity_into_scene(TagHash tag, glm::quat quat, glm::vec4 pos)
{
    ...
    for (auto& resource : entity.resources)
    {
        ...
        SEntityModel model = bin::parse<SEntityModel>(e.MeshFile.data, e.MeshFile.size);

        RenderEntity re{};                           // value-init ? zeros

        // ---- FILL RenderEntity FIELDS (these were never set) ----
        // If your Unk_80806D8F uses different names, map them here.
        re.pos = pos;
        re.rot = quat;
        re.id = tag.hash;
        re.meshData = model;
        ...
        re.external_mats.reserve(e.materials.size());
        for (auto& mat : e.materials) {
